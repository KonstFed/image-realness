...
class CustomDataset(Dataset):
    ...
    def __init__(
        self, data_paths: list[str], labels: list[int], resize: int = None
    ) -> None:
    ...
    def __getitem__(self, index) -> tuple[np.ndarray, int]:
        ...
        img = np.array(img)
        if self._resize:
            img = _resize_with_ratio(img, self._resize)
...
def get_datasets(dataset_path: str, train_test_ratio=0.1, resize=300):
    ...
    train_d = CustomDataset(X_train, y_train, resize=resize)
    test_d = CustomDataset(X_test, y_test, resize=resize)
    shuffle(train_d.data)
    shuffle(test_d.data)
    return train_d, test_d


def get_full_dataset(dataset_path: str, resize=300):
    living_names = os.listdir(os.path.join(dataset_path, "living"))
    fake_names = os.listdir(os.path.join(dataset_path, "fake"))
    limiter = min(len(living_names), len(fake_names))
    living_names = living_names[:limiter]
    ...
    x = [x for x in living_path] + [x for x in fake_path]
    labels = [1] * len(living_path) + [0] * len(fake_path)
    train_d = CustomDataset(x, labels, resize=resize)
    shuffle(train_d.data)
    return train_d
